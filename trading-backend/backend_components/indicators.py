...
def calculate_bollinger_bands(df, period, std_dev):
    ...
    df = pd.concat([df, bb], axis=1)  # Adds upper, middle, lower bands
    return df
...
def calculate_macd(df, fast_period, slow_period, signal_period):
    ...
    df = pd.concat([df, macd], axis=1)  # Adds MACD line and signal line
    return df
...
def calculate_stochastic_oscillator(df, k_period, d_period):
    ...
    df = pd.concat([df, stoch], axis=1)  # Adds %K and %D lines
    return df
...
def calculate_donchian_channels(df, period):
    ...
    df = pd.concat([df, dc], axis=1)  # Adds upper, middle, and lower Donchian channels
    return df
